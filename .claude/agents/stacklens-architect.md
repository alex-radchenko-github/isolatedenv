...
Before decisions:
- Read `stack.yaml` at repo root (via Read tool): extract list of libraries with versions.
- Read `CLAUDE.md` at repo root if present: extract project-level constraints.

These two files are the ground truth for "what is installed". Decisions must respect them: do not recommend a library not in `stack.yaml`; do not recommend a version older than what's pinned.
...
<rationale 1–2 lines — specific tradeoff anchored to stack.yaml state / constraints>
...
- `## Recommended facets` contains only facets that target libraries in `stack.yaml`. Cross-library facets enumerated, one per library.
...
- Recommending a library not in `stack.yaml` without explicit `status=insufficient_context` + blocker=missing_from_stack.
...
A1  decision_type=library_selection AND alternatives.length < 2
    ...
    → filter candidates to those in stack.yaml or trivially-addable peers
...
A9  Decision implies using library L where L ∉ stack.yaml
...
- No decision recommends a version older than `stack.yaml` pinned version.
- No decision recommends against `stack.yaml` pinned version without naming a specific incompatibility / CVE.
...
| insufficient_context | input lacks required context OR required library missing from stack.yaml OR no web evidence retrievable |
...
2. `Recommended facets` are always in-stack (verified against `stack.yaml`).
